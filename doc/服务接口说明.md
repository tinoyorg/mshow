...
- 更新内容：
  - 规范了参数格式和有关单词，
  - 添加使用了TOKEN进行用户认证，token存放在header中
  - 规范区分的参数和body数据，header数据

**接口说明**

- **参数** : 需要在url中使用到的参数,示例如下
...
```
token ：eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJvNVR6NzRzOTlHb2U1NThiSEREek90T0t1MUMwIiwic2Vzc2lvbl9rZXkiOiJ3dzBlQjhYV0xmSDFkZXZjcFdiSU9BPT0ifQ.Gr6vUzB2IiZm8hQpXYeT6N_e51oNaAf820ExOKJOiCU
```
...
- **type** : String 搜索分类（展馆exhibition、展览show、展品collection）
...
POST	/user/object_type/{object_type}/object_id/{object_id}/like
...
DELETE	/user/object_type/{object_type}/object_id/{object_id}/like
...
POST	/user/object_type/{object_type}/object_id/{object_id}/star
...
DELETE	/user/object_type/{object_type}/object_id/{object_id}/star/{star_id}
...
GET  /user/share/{object_id}/type/{object_type}
...
POST	/user/object_type/{object_type}/object_id/{object_id}/report
...
PUT	/show/{show_id}/appointment_info/{appointment_id}
